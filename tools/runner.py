...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # parameter quantity calculationtotal 
    # params = 0
    # for param in base_model.parameters():
    #     params += param.numel()
    # print(f"模型参数数里:{params}")
    # sys.exit()
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            sparse_loss, dense_loss = get_loss(ret, gt, epoch)
        ...
        # train loss fig set
        temp = ['%.4f' % l for l in losses.avg()]
        sum_losses = sum(float(l) for l in temp)
        ...
        plt.grid(True)
        # save img
        plt.savefig(args.experiment_path + '/training_loss.png')
        plt.close() 
