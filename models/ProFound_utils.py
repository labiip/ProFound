...
class SelfAttention(nn.Module):  
    ...
    def forward(self, x):  
        ...
        # # Query, Key, Value  
        # query = self.query(x).view(batch_size, -1, seq_len)  # (B, 128, 512)  
        # key = self.key(x).view(batch_size, -1, seq_len)  # (B, 128, 512) 
        # value = self.value(x).view(batch_size, -1, seq_len)  # (B, 128, 512)
        query = x  # (B, 128, 512)  
        key = x  # (B, 128, 512) 
        value = x  # (B, 128, 512)

        # Attention Scores  
        scores = torch.matmul(query, key.transpose(-2, -1)) / (512 ** 0.5)  # Batch Matrix Multiplication: (B, 128, 512) x (B, 512, 128) -> (B, 128, 128)  
        attention = self.softmax(scores)  # (B, 128, 128)  
        attention = self.dropout(attention)  # Apply dropout  
          
        # Context Vector  
        context = torch.matmul(attention, value)  # (B, 128, 512)
          
        # Output  
        output = x + context  # Residual connection  

        return output

# boundary-awareness
def batch_process_point_clouds(batch_points, k=10, percentile_threshold=90):
    """
    Batch process a set of point cloud data to compute boundary points for each cloud.

    :param batch_points: PyTorch tensor with shape (B, N, 3), representing a batch of point cloud data
    :param k: Number of nearest neighbors
    :param percentile_threshold: Percentile threshold to differentiate boundary points
    :return: List of boundary points for each point cloud
    """
    boundary_points_batch = []

    for i, points in enumerate(batch_points):
        points_np = points.cpu().numpy()  # Convert to NumPy for KDTree

        # Step 1: Build KD-Tree
        kd_tree = KDTree(points_np)

        # Step 2: Compute the average neighbor distance for each point
        average_distances = average_neighbor_distances(points_np, kd_tree, k=k)

        # Step 3: Analyze the distribution of average neighbor distances and set a threshold
        threshold_distance = np.percentile(average_distances, percentile_threshold)

        # Step 4: Identify points with average neighbor distance exceeding the threshold (boundary points)
        boundary_indices = torch.tensor(average_distances > threshold_distance)  # Convert to PyTorch tensor
        boundary_points = points[boundary_indices]
        boundary_points_batch.append(boundary_points)
        
        # Step 5: optional: save boundary points as PCD
        # boundary_points = boundary_points.cpu().numpy()
        # point_cloud = o3d.geometry.PointCloud()
        # point_cloud.points = o3d.utility.Vector3dVector(boundary_points)
        # o3d.io.write_point_cloud(f"output_batch_{i}.pcd", point_cloud)
        # print(f"PCD saved to: output_batch_{i}.pcd")

    return boundary_points_batch

def average_neighbor_distances(points, kd_tree, k=5):
    N = points.shape[0]
    distances, _ = kd_tree.query(points, k=k + 1)  # +1 because the first neighbor is the point itself
    average_distances = np.mean(distances[:, 1:], axis=1)  # Exclude the point itself (first column)
    return average_distances

# midpoint_inter
def midpoint_interpolate(sparse_pts, up_rate):
    # sparse_pts: (b, 3, n)
    ...
    k = int(2 * up_rate)
    # (b, 3, n, k)
    knn_pts = get_knn_pts(sparse_pts, sparse_pts,k)
    # (b, 3, n, k)
    ...
    # fps: (b, 3, up_pts_num)
    interpolated_pts = interpolated_pts.permute(0, 2, 1).contiguous()
    interpolated_pts = fps_subsample(interpolated_pts, up_pts_num)

    return interpolated_pts

def get_knn_pts(sparse_pts, reference_pts, k):
    """
    获取每个点的k近邻点

    参数:
    - k: int, 最近邻点的个数
    - sparse_pts: torch.Tensor, 输入点云, 形状为 (b, 3, n)
    - reference_pts: torch.Tensor, 用于查找邻居的参考点云, 形状为 (b, 3, m)

    返回:
    - knn_pts: torch.Tensor, 每个点的k近邻点, 形状为 (b, 3, n, k)
    """
    # 获取 batch size 和点的数量
    b, c, n = sparse_pts.shape
    _, _, m = reference_pts.shape

    # 计算两两点之间的距离
    # sparse_pts: (b, 3, n) -> (b, n, 3)
    # reference_pts: (b, 3, m) -> (b, m, 3)
    # dist: (b, n, m)
    dist = torch.cdist(sparse_pts.transpose(1, 2), reference_pts.transpose(1, 2))

    # 取每个点的 k 个最近邻点的索引
    knn_idx = torch.topk(dist, k, dim=-1, largest=False).indices  # (b, n, k)

    # 根据索引获取最近邻点
    # reference_pts: (b, 3, m)
    # knn_idx: (b, n, k) -> (b, 1, n, k)
    knn_pts = torch.gather(reference_pts.unsqueeze(2).expand(-1, -1, n, -1), 3, knn_idx.unsqueeze(1).expand(-1, c, -1, -1))

    return knn_pts

# save point cloud as PCD
def save_tensor_to_pcd(tensor, output_dir):  
    """  
    将形状为 (batch_size, num_points, 3) 的张量保存为多个 PCD 文件。  
  
    参数:  
    - tensor: torch.Tensor, 输入的点云数据，形状为 (batch_size, num_points, 3)  
    ...
    for i in range(batch_size):  
        # 获取每个点云的 (num_points, 3) 数据，并确保数据类型为 np.float64  
        point_cloud_np = tensor[i].cpu().numpy().astype(np.float64)  
        ...
        # 保存为 PCD 文件  
        output_file_path = os.path.join(output_dir, f"point_cloud_{i}.pcd")  
        o3d.io.write_point_cloud(output_file_path, pcd)  
        print(f"Saved {output_file_path}")

def save_point_clouds_as_pcd(data, prefix='point_cloud'):
    """
    保存点云数据为 PCD 文件。

    参数:
    - data: 输入的点云数据，形状为 (N, 3, 128)
    - prefix: 文件名前缀
    """
    # 确保输入数据在 CPU 上
    data = data.cpu()
    ...
    for i in range(data.size(0)):
        points = data[i].detach().transpose(0, 1).numpy()  # 使用 detach() 进行转换
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(points)
        o3d.io.write_point_cloud(f'{prefix}_{i}.pcd', pcd)
